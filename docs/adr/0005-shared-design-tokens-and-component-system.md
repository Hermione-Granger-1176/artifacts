...
5. `scripts/lint/check_app_css_tokens.py`, exposed as `make lint-app-css-tokens` and wired into `lint` and `ci-web`, forbids hex colors and literal `rgb()` / `rgba()` colors in `apps/*/css/*.css` and enforces token usage for `border-radius`, `font-size`, and `letter-spacing`, with small documented allowlists for deliberate sub-token values.
