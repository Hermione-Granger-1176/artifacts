...
7. **Environment caching, layered.** Jobs cache the Playwright browser directory and, as the primary layer, `.venv` and `node_modules` directly, keyed exactly on lockfile hashes with no restore-keys, so a hit skips dependency installation entirely and a miss installs from scratch. The download caches behind them (`~/.cache/uv`, `~/.npm`) are the _secondary_ layer and are restored only when their consumer will run, that is when the matching `.venv` or `node_modules` key missed. Restoring a download cache next to a materialized hit transfers an archive that no step then opens. There is deliberately no `setup-python` pip cache: Python dependencies reach `.venv` through uv, so pip has no role on the hit path, and a `hashFiles('uv.lock')` key made it worse than useless, because `actions/cache` skips the save on a primary-key hit and the entry therefore froze at whatever it held when `uv.lock` last changed. The one pip install that remains (the uv binary) has its own small cache keyed on the pinned `uv-version` input, so the key rolls exactly when the stored wheel does. The `.venv` key includes the resolved interpreter version and runner architecture (not the bare version input) so runner image updates roll the cache instead of restoring a venv built against a missing toolcache path. The uv binary is installed unconditionally because make targets such as `audit-python` invoke uv directly; only the dependency install is skippable. On a Playwright cache hit the setup skips the apt system-deps pass (`make setup-playwright` instead of `make setup-playwright-ci`); browser tests fail loudly if system libraries were ever missing.
