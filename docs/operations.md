...
- If a cache key is rolled or retired, the old entries are not removed automatically and keep counting against the repository's cache budget. List them with `make ci-caches key=<prefix>` and drop each with `make ci-cache-delete cache=<id|key>`. Do this only after the change that retires the key is on `main`, since a run on the old code recreates the entry.
