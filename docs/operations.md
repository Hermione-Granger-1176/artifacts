...
- `make lint-app-css-tokens` verifies that every `apps/<slug>/css/*.css` stays on the shared design tokens: no hex colors, no `rgb()` / `rgba()` colors that are not derived from `var()` or `color-mix()`, no `border-radius` px literals above 5px (use `var(--radius-*)`), no raw px font sizes (use `var(--font-size-*)` or relative units, including inside `clamp()`), and `letter-spacing` that is exactly one `var(--tracking-*)` token or `normal`. Small documented allowlists in the checker grandfather a few deliberate literals.
