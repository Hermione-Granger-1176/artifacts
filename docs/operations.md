...
- `make security` mirrors the practical local dependency audits in CI; the Python side runs a policy-driven audit over the configured lock files in `config/security_audit.json`, while Gitleaks and GitHub dependency review remain CI-only because this repo does not vendor those scanners locally.
