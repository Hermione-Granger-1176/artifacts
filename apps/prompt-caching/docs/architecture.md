...
1. `index.html` loads `js/app-theme.js` (synchronous theme bootstrap), the shared
   `app-tokens.css` + `app-shell.css`, then the app-local `css/app.css`.
...
App-local accents (`--pc-accent/warm/teal/indigo/rose`) are mapped onto the shared
semantic palette (`--color-amber/blue/green/purple/red`) at the top of `css/app.css`.
