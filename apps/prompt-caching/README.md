...
inference pipeline — tokenizer → embeddings → attention → KV cache → providers —
and shows that caching saves the attention mechanism's **K and V matrices**, not
...
- `index.html` — semantic layout inside the shared app shell (no inline styles/scripts)
- `css/app.css` — app-local styling, built entirely on shared design tokens
- `js/app.js` — entry point; wires the shared shell and the feature modules
...
- None. No CDN scripts or web fonts; everything resolves to shared app tokens and
  the system font stacks, satisfying the strict `'self'` CSP.

## Development

- Keep shared design decisions in the root app system; keep app-local changes here.
