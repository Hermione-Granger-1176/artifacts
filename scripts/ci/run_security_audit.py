#!/usr/bin/env python3
"""Run policy-driven dependency security audits for exported Python requirements."""
...
def _relative_path(path: Path) -> str:
    """Return a repo-relative path string, or the original path when outside the repo."""
    try:
...
def _run_pip_audit(requirements_file: Path) -> tuple[VulnerabilityFinding, ...]:
    """Run pip-audit for one requirements file and return all reported vulnerabilities."""
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    ignored, errors = _audit_python_dependencies(
        today=date.today(), exceptions=exceptions, requirements_file=requirements_file
    )
