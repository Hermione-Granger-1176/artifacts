...
Each artifact directory can contain:
    ...
    - thumbnail.png: Auto-generated by generate_thumbnails.py.
...
def _extract_artifact(folder: Path) -> ArtifactItem | None:
    ...
    if (folder / THUMBNAIL_FILE).exists():
        thumbnail = f"apps/{folder.name}/{THUMBNAIL_FILE}"
...
def _build_artifacts_table(items: list[ArtifactItem]) -> str:
    """Build the README artifacts table."""
    lines = [
        "| Artifact | Description | Tools | Live Link |",
        "|----------|-------------|-------|-----------|",
    ]
    for item in items:
        tools_str = ", ".join(t.capitalize() for t in item["tools"])
        link = f"https://hermione-granger-1176.github.io/artifacts/{item['url']}"  # url already includes apps/
        lines.append(
            f"| {item['name']} | {item['description']} | {tools_str} | [Open]({link}) |"
        )
    return "\n".join(lines)
...
def _update_readme(items: list[ArtifactItem]) -> None:
    ...
    readme = _replace_block_marker(
        readme, "ARTIFACTS_TABLE", _build_artifacts_table(items)
    )
