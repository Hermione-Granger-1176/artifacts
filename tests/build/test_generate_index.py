...
def test_read_artifact_contract_loads_shared_validation_rules(
    ...
    monkeypatch.setattr(generate_index, "ARTIFACT_CONTRACT_FILE", contract_file)
    generate_index._read_artifact_contract_file.cache_clear()
    generate_index._artifact_id_pattern.cache_clear()

    try:
        contract = generate_index._artifact_contract()
    finally:
        generate_index._read_artifact_contract_file.cache_clear()
        generate_index._artifact_id_pattern.cache_clear()
...
def test_read_artifact_contract_rejects_invalid_contract_shapes(
    ...
    try:
        with pytest.raises(ValueError, match=message):
            generate_index._artifact_contract()
    finally:
        generate_index._read_artifact_contract_file.cache_clear()
        generate_index._artifact_id_pattern.cache_clear()
...
""".strip(),
    ...
    monkeypatch.setattr(generate_index, "ROOT_GALLERY_FOUNDATION_FILE", tokens_file)
...
def test_fallback_badge_color_uses_default_when_palette_file_is_missing(
    ...
    monkeypatch.setattr(
        ...
        tmp_path / "css" / "gallery" / "missing-tokens.css",
    )
...
def test_extract_artifact_builds_expected_structure(tmp_path: Path) -> None:
    artifact_dir = create_artifact(
        ...
        thumbnail_file=generate_index._artifact_contract()["thumbnailFile"],
    )
    ...
    assert item == {
        ...
        "tools": ["claude", "chatgpt"],
        "url": "apps/loan-tool/",
        "thumbnail": generate_index._artifact_thumbnail_path("loan-tool"),
    }
...
def test_frontend_config_contains_display_labels() -> None:
    ...
    assert frontend_config["artifactContract"] == generate_index._artifact_contract()
...
def test_generate_writes_js_output_and_updates_readme(
    ...
    assert payload[0]["thumbnail"] == generate_index._artifact_thumbnail_path(
        "loan-tool"
    )
...
def test_helper_wrappers_delegate_expected_contract_and_metadata_logic() -> None:
    assert generate_index._artifact_url_rule() == "apps/<artifact-id>/"
    assert (
        generate_index._artifact_thumbnail_rule() == "apps/<artifact-id>/thumbnail.webp"
    )
    assert generate_index._matches_artifact_url_shape("apps/loan-tool/") is True
    assert (
        generate_index._matches_artifact_thumbnail_shape(
            "apps/loan-tool/thumbnail.webp"
        )
        is True
    )
    generate_index._validate_relative_repo_path(
        "apps/loan-tool/thumbnail.webp",
        field_name="Artifact thumbnail",
    )
    generate_index._validate_gallery_metadata_entries(
        "tags",
        [
            {
                "id": "finance",
    ...
    assert generate_index._format_identifier_words("ai-ui") == ["AI", "UI"]
    assert generate_index._sort_items({"beta", "alpha"}, ["beta"]) == ["beta", "alpha"]
