...
def build_real_site(tmp_path: Path, monkeypatch) -> Path:
    ...
    monkeypatch.setattr(prepare_site, "PYPROJECT_FILE", source_root / "pyproject.toml")
    monkeypatch.setattr(prepare_site, "DEPLOY_DIR", deploy_root)
    monkeypatch.setenv(prepare_site.DEPLOY_VERSION_ENV_VAR, "apptest")
    monkeypatch.setenv(prepare_site.DEPLOY_COMMIT_SHA_ENV_VAR, "apptest" * 5)
...
class RuntimeMonitor:
    ...
    def bind(self, page) -> None:
        ...
        def track_console(message) -> None:
            ...
            if (
                ...
                and _is_duplicate_css_fetch(location_url, 404)
            ):
                return
    ...
    def _should_ignore_url(self, url: str) -> bool:
        ...
        HTTP(S) hosts. Failed or errored requests to external origins are recorded
        by the request/response tracking callbacks, which is the intended
        enforcement point for the self-hosting policy.
