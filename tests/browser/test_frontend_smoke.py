...
def test_gallery_smoke_covers_root_interactions(tmp_path: Path, monkeypatch) -> None:
    """Test gallery smoke covers root interactions."""
...
def test_404_links_handle_root_and_preview_paths(tmp_path: Path, monkeypatch) -> None:
    """Test 404 links handle root and preview paths."""
    deploy_root = build_smoke_site(tmp_path, monkeypatch)
    ...
    with (
        StaticServer(deploy_root) as server,
        sync_playwright() as playwright,
        MonitoredPage(playwright, server.url, name="frontend-smoke-404") as session,
    ):
        page = session.page
        assert page is not None
        ...
        expect(page.locator("#home-link")).to_have_attribute("href", "/pr-preview/pr-42/")


def test_gallery_shows_runtime_error_for_invalid_bootstrap_data(
    tmp_path: Path, monkeypatch
) -> None:
    """Test gallery shows runtime error for invalid bootstrap data."""
