...
def test_root_page_has_no_blocking_axe_violations_in_light_theme(
    ...
    """Test root page has no blocking axe violations in light theme."""
...
def test_root_page_has_no_blocking_axe_violations_in_dark_theme(
    ...
    """Test root page has no blocking axe violations in dark theme."""
    deploy_root = build_smoke_site(tmp_path, monkeypatch)
...
def test_overlay_and_no_results_states_have_no_blocking_axe_violations(
    ...
    """Test overlay and no results states have no blocking axe violations."""
    deploy_root = build_smoke_site(tmp_path, monkeypatch)
...
def test_404_page_has_no_blocking_axe_violations_and_good_contrast(
    ...
    """Test 404 page has no blocking axe violations and good contrast."""
    deploy_root = build_smoke_site(tmp_path, monkeypatch)

    with (
        StaticServer(deploy_root) as server,
        sync_playwright() as playwright,
        MonitoredPage(playwright, server.url, name="a11y-404", bypass_csp=True) as session,
    ):
        page = session.page
        assert page is not None
