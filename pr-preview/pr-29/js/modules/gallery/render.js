import{getPageNumbers as O}from"./catalog.js";export{escapeHtml}from"../html-escape.js";import{escapeHtml as n}from"../html-escape.js";import{ICONS as f}from"./icons.js";const C=["var(--card-color-1)","var(--card-color-2)","var(--card-color-3)","var(--card-color-4)","var(--card-color-5)","var(--card-color-6)","var(--card-color-7)","var(--card-color-8)","var(--card-color-9)","var(--card-color-10)","var(--card-color-11)","var(--card-color-12)"],S=["var(--color-note-1)","var(--color-note-2)","var(--color-note-3)","var(--color-note-4)","var(--color-note-5)","var(--color-note-6)"],N=["-1.4deg","0.6deg","-0.4deg","1.2deg","-0.9deg","1.5deg","0.3deg","-1.1deg","0.8deg","-0.5deg","1.3deg","-0.7deg"],T=["-0.35deg","0.2deg","-0.12deg","0.4deg","-0.25deg","0.45deg","0.1deg","-0.3deg","0.25deg","-0.15deg","0.36deg","-0.2deg"];function F(a){return C[a%C.length]}function H(a){const t=a%N.length;return{noteRotate:N[t],noteHoverRotate:T[t]}}const h=new Map;let p=null;function L(){if(!p){p=[...S];for(let a=p.length-1;a>0;a-=1){const t=Math.floor(Math.random()*(a+1));[p[a],p[t]]=[p[t],p[a]]}}return p}function u({active:a=!1,className:t,color:e,datasetName:s,datasetValue:i,label:r,rotate:d=null,surface:c}){const l=d!==null?` data-rotate="${d}"`:"";return`<button class="${t}${a?" is-active":""}" data-filter-surface="${c}" ${s}="${n(i)}" data-chip-color="${n(e)}"${l} type="button" aria-controls="artifacts-grid" aria-pressed="${a}">${n(r)}</button>`}function V(a){return h.get(a)||"var(--color-capsule-default)"}function A(a,t,e=""){return!Array.isArray(a)||a.length===0?e:`
  ...
  `}export function createDetailContent(a){const t=a.thumbnail?`<img class="detail-media" src="${n(a.thumbnail)}" alt="${n(a.name)} preview">`:'<div class="detail-media-placeholder"></div>',e=a.description||"Open the artifact to explore the interactive experience.",s=A(a.tags,"detail-meta-tags"),i=A(a.tools,"detail-meta-tools");return`
    ...
    <div class="detail-content">
      <h2 id="detail-title" class="detail-title">${n(a.name)}</h2>
      <p id="detail-description" class="detail-description">${n(e)}</p>
      ${s||i?`<div class="detail-meta">${s}${i}</div>`:""}
      <a class="detail-open-link" href="${n(a.url)}" target="_blank" rel="noopener noreferrer"
  ...
  `}function M(a,t,e){const s=F(e),i=a.thumbnail?`
      <div class="card-photo-frame">
        <img class="card-thumbnail" src="${n(a.thumbnail)}" alt="${n(a.name)}" loading="lazy">
      </div>
    `:'<div class="card-thumbnail-placeholder"></div>',r=H(e);return`
    <button class="artifact-card ${t?"expanded":""}" data-id="${n(a.id)}" data-card-color="${s}" data-note-rotate="${r.noteRotate}" data-note-hover-rotate="${r.noteHoverRotate}" type="button"
      aria-label="View details for ${n(a.name)}" aria-expanded="${t}" aria-haspopup="dialog">
      <div class="card-note">
        <div class="card-thumbnail-area">
          ${i}
        </div>
        <div class="card-overlay card-note-body">
          <div class="card-name">${n(a.name)}</div>
        </div>
      </div>
    </button>
  `}export function buildGridHtml(a,t){const e=a.map((r,d)=>M(r,t===r.id,d)),s=e.filter((r,d)=>d%2===0),i=e.filter((r,d)=>d%2!==0);return`
