import{getPageNumbers as F}from"./catalog.js";export{escapeHtml}from"../html-escape.js";import{escapeHtml as r}from"../html-escape.js";import{ICONS as f}from"./icons.js";const N=["var(--card-color-1)","var(--card-color-2)","var(--card-color-3)","var(--card-color-4)","var(--card-color-5)","var(--card-color-6)","var(--card-color-7)","var(--card-color-8)","var(--card-color-9)","var(--card-color-10)","var(--card-color-11)","var(--card-color-12)"],L=["var(--color-note-1)","var(--color-note-2)","var(--color-note-3)","var(--color-note-4)","var(--color-note-5)","var(--color-note-6)"],x=["-1.4deg","0.6deg","-0.4deg","1.2deg","-0.9deg","1.5deg","0.3deg","-1.1deg","0.8deg","-0.5deg","1.3deg","-0.7deg"],S=["-0.35deg","0.2deg","-0.12deg","0.4deg","-0.25deg","0.45deg","0.1deg","-0.3deg","0.25deg","-0.15deg","0.36deg","-0.2deg"];function H(a){return N[a%N.length]}function M(a){const e=a%x.length;return{noteRotate:x[e],noteHoverRotate:S[e]}}const y=new Map;let p=null;function V(){if(!p){p=[...L];for(let a=p.length-1;a>0;a-=1){const e=Math.floor(Math.random()*(a+1));[p[a],p[e]]=[p[e],p[a]]}}return p}function u({active:a=!1,className:e,color:t,datasetName:l,datasetValue:s,label:i,rotate:d=null,surface:c}){const n=d!==null?` data-rotate="${d}"`:"";return`<button class="${e}${a?" is-active":""}" data-filter-surface="${c}" ${l}="${r(s)}" data-chip-color="${r(t)}"${n} type="button" aria-controls="artifacts-grid" aria-pressed="${a}">${r(i)}</button>`}function w(a){return y.get(a)||"var(--color-capsule-default)"}function A(a,e,t=""){return!Array.isArray(a)||a.length===0?t:`
    <div class="${e}">
      ${a.slice(0,3).map(l=>`<span class="${e}-item" data-capsule-bg="${r(w(l))}">${r(l)}</span>`).join("")}
    </div>
  `}export function buildFilterNotes({tools:a,tags:e,activeTools:t,activeTags:l,toolLabel:s,tagLabel:i}){let d=1;const c=()=>{const o=Math.sin(d++)*1e4;return o-Math.floor(o)},n=V(),g=o=>n[(o+1)%n.length],C=a.length+1,v=o=>n[(C+o)%n.length],h=t.length>0,$=l.length>0,k=[u({active:!h,className:"desk-note",color:n[0],datasetName:"data-filter-note",datasetValue:"all-tools",label:"All",rotate:(c()*6-3).toFixed(1),surface:"desk"}),...a.map((o,b)=>{const m=g(b);return y.set(o,m),u({active:t.includes(o),className:"desk-note",color:m,datasetName:"data-filter-tool",datasetValue:o,label:s(o),rotate:(c()*8-4).toFixed(1),surface:"desk"})})],R=[u({active:!$,className:"desk-note",color:n[0],datasetName:"data-filter-note",datasetValue:"all-tags",label:"All",rotate:(c()*6-3).toFixed(1),surface:"desk"}),...e.map((o,b)=>{const m=v(b);return y.set(o,m),u({active:l.includes(o),className:"desk-note",color:m,datasetName:"data-filter-tag",datasetValue:o,label:i(o),rotate:(c()*8-4).toFixed(1),surface:"desk"})})],O=[u({active:!h,className:"mobile-filter-chip",color:n[0],datasetName:"data-filter-note",datasetValue:"all-tools",label:"All tools",surface:"mobile"}),...a.map((o,b)=>u({active:t.includes(o),className:"mobile-filter-chip",color:g(b),datasetName:"data-filter-tool",datasetValue:o,label:s(o),surface:"mobile"}))],T=[u({active:!$,className:"mobile-filter-chip",color:n[0],datasetName:"data-filter-note",datasetValue:"all-tags",label:"All tags",surface:"mobile"}),...e.map((o,b)=>u({active:l.includes(o),className:"mobile-filter-chip",color:v(b),datasetName:"data-filter-tag",datasetValue:o,label:i(o),surface:"mobile"}))];return`
  ...
  `}export function handleThumbnailError(a){const e=a.target;if(!e||e.tagName!=="IMG"||!e.classList.contains("card-thumbnail"))return;const t=e.closest(".card-photo-frame"),l=t?.parentNode;if(!t||!l)return;const s=t.ownerDocument.createElement("div");s.className="card-thumbnail-placeholder",l.replaceChild(s,t)}export function registerThumbnailFallback(a){a.addEventListener("error",handleThumbnailError,!0)}function E(a,e,t){const l=H(t),s=a.thumbnail?`
      <div class="card-photo-frame">
        <img class="card-thumbnail" src="${r(a.thumbnail)}" alt="${r(a.name)}" loading="lazy">
      </div>
    `:'<div class="card-thumbnail-placeholder"></div>',i=M(t);return`
  ...
  `}export function buildGridHtml(a,e){const t=a.map((i,d)=>E(i,e===i.id,d)),l=t.filter((i,d)=>d%2===0),s=t.filter((i,d)=>d%2!==0);return`
