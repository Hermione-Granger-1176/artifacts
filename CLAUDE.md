...
- The bookmark-note palette is the shared color system. App CSS colors must be token-derived (`var()` or a `color-mix()` over tokens), never hex or literal `rgb()` / `rgba()`; raw `rgb()` / `rgba()` values belong in the `css/src/` token definitions. `make lint-app-css-tokens` enforces this plus token usage for radius, font-size, and letter-spacing
